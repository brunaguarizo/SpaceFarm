...
function handleKeyPress(e) {
    ...
    const moveDistance = 40; // Increased from 20 to 40 for bigger movements

    if (e.key === "ArrowUp" || e.key === "w") {
        // Move up
        playerY = Math.max(70, playerY - moveDistance);
        updatePlayerPosition();
    } else if (e.key === "ArrowDown" || e.key === "s") {
        // Move down
        playerY = Math.min(gameHeight - 70, playerY + moveDistance);
        updatePlayerPosition();
    }
}
